...
import 'operation/network_error_manager.dart';
import 'package:flutter/foundation.dart' show compute;

import 'package:retry/retry.dart';

import '../src/feature/ssl/io_custom_override.dart' as ssl;
...
import 'utility/json_encode_util.dart';
...
class NetworkManager<E extends INetworkModel<E>?>
  ...
  E Function(Map<String, dynamic> data, int? statusCode)? errorModelFromData;
  ...
  Future<IResponseModel<R?, E?>> send<T extends INetworkModel<T>, R>(
    ...
    final cacheData = await _getCacheData<R, T>(
      expiration,
      method,
      parseModel,
    );
    ...
    try {
      ...
      if (responseStatusCode >= HttpStatus.ok &&
        ...
        return _getResponseResult<T, R>(
            response0, parseModel, response.statusCode ?? 500);
      } else {
  ...
  ResponseModel<R, E> _onError<R>(dio.DioException e, bool isErrorDialog) {
    ...
    if (errorResponse != null) {
      error = _generateErrorModel(error, errorResponse.data, isErrorDialog);
    }
    return ResponseModel<R, E>(
      response: e.response,
  ...
  ErrorModel<E> _generateErrorModel(
      ErrorModel<E> error, dynamic data, bool isErrorDialog) {
    ...
    if (data is String) {
      ...
      if (errorModelFromData != null) {
        if (isErrorDialog) {
          generatedError = error.copyWith(
            model: errorModelFromData?.call(jsonBody, error.statusCode),
          );
        }
      } else {
